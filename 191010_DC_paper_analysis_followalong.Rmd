...
ggplot(data3, aes(x=Celltype, y=log2(Counts+1),fill=proximity))+geom_boxplot(outlier.size=.2)+stat_compare_means(method="wilcox.test", aes(label=symnum(..p.adj.., cutpoints = c(0,.00005,  .0005,.005,.05,  1), symbols = c("****","***","**","*","ns") )))+theme_classic()+scale_fill_discrete(name="Distance to closet \nChromatin Entry Site (CES)")+theme(axis.text.x=element_text(size=10))+ 
    scale_x_discrete(labels = get_wraper(12))
