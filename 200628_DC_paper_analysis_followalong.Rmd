...
testisspecificgenes<-testisspecificgenes$Gene
...
p1<-plot_grid(p1, p2, ncol=2, rel_widths=c(1,1), labels=c("C", "D"), label_size = 10)
#Figure 1B- make t-sNE showing dimensional reduction of cell types
fig1b<-DimPlot(testis.integrated, reduction = "tsne", label = FALSE , cols = pal)
p2 <- plot_grid(NULL, fig1b, labels=c("A", "B"), rel_widths=c(0.8,1),label_size=10)
p2<-p2+theme(text=element_text(size=10), axis.title=element_text(size=10), legend.text=element_text(size=10))
p1<-p1+theme(text=element_text(size=10), axis.title=element_text(size=10), legend.text=element_text(size=10))

p1 <-plot_grid(p2, p1, ncol=1,rel_widths = c(1.2,1))

 
#plot_grid(p1, p2, ncol=1, labels = c("A", "C"), label_size = 10, rel_heights = c(0.8,1.5))
p1
#ggsave( "~/Box Sync/Evan/Zhao lab notebook/Writing/DC paper revisions/200325_figure1.pdf", width=10, height=8)
#note: I added some stuff in illustrator
```
Time for figure 2:
...
flyCDS$Genename<-flyCDS$SYMBOL
#position vs counts: save for rainy day
posvscounts<-join(subset(flyCDS, SYMBOL %in%X),data2 )
ncell<-data.frame(table(testis.integrated@active.ident))
colnames(ncell)<-c("Celltype", "ncell")
posvscounts<-join(posvscounts, ncell)
posvscounts$countspercell<-posvscounts$Counts/posvscounts$ncell
posvscounts$logcountspercell<-log2(posvscounts$countspercell+1)

posplot<-ggplot(posvscounts, aes(x=start, y=countspercell))+stat_summary_bin(fun.y="sum", geom="line", bins=2354)+facet_wrap(~Celltype, scales="free", nrow = 9)+theme_classic()+xlab("Gene start position (bp)")+ylab("Counts per cell")

posplot<-ggplot(posvscounts, aes(x=start, y=countspercell))+stat_summary_bin(fun.y="sum", geom="line", bins=235.4)+facet_wrap(~Celltype, scales="free", nrow = 9)+theme_classic()+xlab("Gene start position (bp)")+ylab("Counts per cell")

posplot<-ggplot(posvscounts, aes(x=start, y=countspercell))+stat_summary_bin(fun.y="mean", geom="line", bins=50)+facet_wrap(~Celltype, scales="free", nrow = 9)+theme_classic()+xlab("Gene start position (bp)")+ylab("Counts per cell")



#need to correlate faceted independent and dependent variables
```
...
plot_grid(p7,p8,p1,p2,p3,p4, nrow=2)



#flyatlas expression#supplemental figure 14

flyatlas<-read.csv("~/Downloads/WittZhao2020data/DCC-gene-fpkm.csv")
